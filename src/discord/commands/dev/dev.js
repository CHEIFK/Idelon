    ...
    try {
      ...
      switch (subcommand) {
        case 'give-item': {
          ...
          const amount = rawAmount === null || rawAmount === undefined ? 1 : rawAmount;
          result = await devService.giveItem(adminId, targetUser, item, amount);
          if (!result.success) return { embed: createErrorEmbed('Dev Action Error', result.message || result.reason) };
